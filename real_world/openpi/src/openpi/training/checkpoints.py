...
def prune_old_train_states(
    checkpoint_manager: ocp.CheckpointManager,
    checkpoint_dir: epath.Path | str,
    latest_step: int,
) -> None:
    # Wait for any in-flight async saves to commit so we don't race with a write.
    checkpoint_manager.wait_until_finished()
    checkpoint_dir = epath.Path(checkpoint_dir)
    for step_dir in checkpoint_dir.iterdir():
        if not step_dir.is_dir() or not step_dir.name.isdigit():
            continue
        if int(step_dir.name) == latest_step:
            continue
        train_state_dir = step_dir / "train_state"
        if train_state_dir.exists():
            train_state_dir.rmtree()
            logging.info(f"Pruned train_state at {train_state_dir}")
