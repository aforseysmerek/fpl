...
# range so the knob is unmistakable: at 0 it's a low (but solid, always-lifted)
# carry, at 1 it's a distinctly high carry. Min 0.10 is above the success margin
# (table+0.10); the arms converge to the commanded height during the lift-hold, so
# realized height should track the knob. If the top saturates (realized < commanded
# high), the arms are reach/sag-limited up there -> dial CARRY_H_RANGE back.
CARRY_H_MIN = 0.10      # height_amount=0 -> low but solid lift + carry
CARRY_H_RANGE = 0.35    # height_amount=1 -> 0.45 lift
...
APPROACH_H = 0.14       # m above the grasp point; high enough that the arms descend
                        # into a reachable elbow config (verified headless)
...
def _natural_yaw(quat_xyzw):
    """The arm's reachable yaw: grip_site local x projected onto the table."""
    R = _RT_Q2M.forward(np.asarray(quat_xyzw, float)[[3, 0, 1, 2]])   # xyzw -> wxyz
    x = R[:, 0].copy(); x[2] = 0.0                                    # local x projected horizontal
    return float(np.arctan2(x[1], x[0]))


def bar_perp_grasp_R(quat_xyzw, h_self, h_other, max_twist=MAX_TWIST):
    ...
    @quat_xyzw is the arm's grip_site quaternion at reset; @h_self / @h_other are
    this arm's handle and the other handle (xy defines the radial direction)."""
    nat = _natural_yaw(quat_xyzw)                                    # natural yaw
...
def finger_bar_deg(quat_xyzw, h_self, h_other):
...
class PotTracePolicy:
    def __init__(self, env, start_eef0, start_eef1, start_quat0, start_quat1,
        ...
        # grip_site quats at reset; the grasp orientations o0/o1 are computed in
        # generate_trajectory, AFTER the handles are snapshotted and assigned to arms
        # (the finger axis is set relative to the radial/bar direction, so it needs
        # to know which handle each arm is going to).
        self.start_quat0 = np.asarray(start_quat0, float)
        self.start_quat1 = np.asarray(start_quat1, float)
    ...
    def generate_trajectory(self):
        ...
        self.q_grasp0 = _RT_M2Q.forward(bar_perp_grasp_R(self.start_quat0, h0, h1))
        self.q_grasp1 = _RT_M2Q.forward(bar_perp_grasp_R(self.start_quat1, h1, h0))
        self.q_start0 = self.start_quat0[[3, 0, 1, 2]]   # reset grip quat, xyzw -> wxyz
        self.q_start1 = self.start_quat1[[3, 0, 1, 2]]
        # per-spawn diagnostics (natural finger-vs-bar angle; <~30 deg = failure band)
        self.finger_bar = (finger_bar_deg(self.start_quat0, h0, h1),
                           finger_bar_deg(self.start_quat1, h1, h0))
        ...
        h0d, h1d = h0 + up * gz, h1 + up * gz                # descend / grasp pose
        lift0, lift1 = h0d + up * self.carry_h, h1d + up * self.carry_h
        above0, above1 = h0 + up * (APPROACH_H + gz), h1 + up * (APPROACH_H + gz)

        # The approach (reach, descend, lift) moves at the SAME linear speed as the
        # carry: each motion segment's duration = its distance / v, where v is the
        # carry's per-step speed. So a fast carry gets a fast (but still smooth, via
        # slerp + smoothstep) reach/descend, and a slow carry a slow one — no more
        # "crawl down then snap." Only the physical dwells (finger close, height
        # settle, goal settle) are fixed, since they aren't travel.
        carry_len = float(np.linalg.norm(lat))
        v = max(carry_len / max(self.carry_steps, 1), 1e-4)   # m per env step
        GRASP_DWELL, LIFT_HOLD, SETTLE = 50, 30, 40           # fixed (physical, not travel)

        def _seg(f0, f1, t0, t1, floor=8):
            d = max(float(np.linalg.norm(np.asarray(t0) - np.asarray(f0))),
                    float(np.linalg.norm(np.asarray(t1) - np.asarray(f1))))
            return int(max(round(d / v), floor))

        # waypoints: (t, pos0, pos1, grip). Positions interpolate between waypoints
        # (eased by smoothstep in predict_action) so the arms stay in sync and smooth.
        wp = []
        t = _seg(self.start_eef0, self.start_eef1, above0, above1)   # reach above + align
        wp.append((t, above0, above1, OPEN))
        self.t_align = t                          # wrist fully slerped to the grasp orientation by here

        t += _seg(above0, above1, h0d, h1d)       # descend to grasp depth
        wp.append((t, h0d, h1d, OPEN))
        ...
        t += _seg(h0d, h1d, lift0, lift1)         # lift straight up (matches carry speed)
        wp.append((t, lift0, lift1, CLOSE))

        t += LIFT_HOLD                            # HOLD at lift height: converge to the commanded height
        wp.append((t, lift0, lift1, CLOSE))       #   BEFORE the carry, so the height knob actually bites
        self.carry_t0 = t                         # carry (and the metric window) start here

        t += self.carry_steps                     # carry to goal (same delta both hands)
        wp.append((t, lift0 + lat, lift1 + lat, CLOSE))
        self.carry_t1 = t

        t += SETTLE                               # settle at goal
        wp.append((t, lift0 + lat, lift1 + lat, CLOSE))

        self.trajectory = [{"t": t, "pos0": p0, "pos1": p1, "grip": g} for (t, p0, p1, g) in wp]
...
def run_one(env, speed_amount, height_amount, grasp_offset, goal_xy, out_mp4, max_tries=6,
    ...
    for attempt in range(1, max_tries + 1):
        ...
        success = bool(all(grasped) and env._check_success())
...
def main(out_dir, speed_amount, height_amount, grasp_offset, goal_x, goal_y, cam_pullback, cam_fovy, spawn_back, sweep):
